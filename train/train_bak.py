...
from utils.mail.email_sender import send_text_email
...
if __name__ == '__main__':
    ...
    # Send log via email after training finishes
    try:
        receiver = "iyo11@qq.com"
        # Read and limit content size to avoid oversized email
        log_text = log_path.read_text(encoding="utf-8", errors="ignore")
        max_chars = 150_000  # adjust if needed
        if len(log_text) > max_chars:
            head = log_text[:80_000]
            tail = log_text[-60_000:]
            log_text = (
                head
                + "\n\n--- LOG TRUNCATED (middle omitted) ---\n\n"
                + tail
            )
        subject = f"Training Finished: {run_name}"
        content = (
            f"Training finished.\n\n"
            f"Run name: {run_name}\n"
