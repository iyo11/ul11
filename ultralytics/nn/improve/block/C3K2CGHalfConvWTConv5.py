...
class HalfConv(nn.Module):
    def __init__(self, dim, n_div=2, k=5):  # <--- 1. 这里增加了 k参数，默认为 5
        super().__init__()
        self.dim_conv3 = dim // n_div
        self.dim_untouched = dim - self.dim_conv3

        # 自动计算 padding，确保输入输出尺寸一致
        # 如果 k=5, p=2; 如果 k=3, p=1
        p = k // 2

        # <--- 2. 修改 Conv2d，传入 k 和 p
        self.partial_conv3 = nn.Conv2d(self.dim_conv3, self.dim_conv3, k, 1, p, bias=False)
...
class CGHalfConv(nn.Module):
    def __init__(self, dim, k=5):  # <--- 3. 这里也可以接收 k 参数
        super(CGHalfConv, self).__init__()
        self.div_dim = int(dim / 3)
        self.remainder_dim = dim % 3

        # <--- 4. 实例化 HalfConv 时传入 k
        self.p1 = HalfConv(self.div_dim, 2, k=k)
        self.p2 = HalfConv(self.div_dim, 2, k=k)
        self.p3 = HalfConv(self.div_dim + self.remainder_dim, 2, k=k)
...
if __name__ == "__main__":
    ...
    # 实例化 C3k2_WTConv
    model = C3k2_CGHalfWTConv5(64, 64, n=1, c3k=True)  # 可以尝试 c3k=True 或 False
