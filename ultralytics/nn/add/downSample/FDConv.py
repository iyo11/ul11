...
class FDConv(nn.Module):
    def __init__(self,
        ...
        self.kernel_temp = kernel_temp
        self.temp = temp if temp is not None else kernel_temp
        self.att_multi = att_multi
        ...
        self.spatial_freq_decompose = spatial_freq_decompose
        self.convert_param = convert_param
        self.linear_mode = linear_mode
        self.fbm_cfg = fbm_cfg
        ...
        # 检查是否应该使用FDConv
        self.use_fdconv = (
                min(self.in_channels, self.out_channels) > self.use_fdconv_if_c_gt and
                self.kernel_size[0] in self.use_fdconv_if_k_in
        )

        if not self.use_fdconv:
            return
        ...
        self.KSM_Global = KernelSpatialModulation_Global(
            ...
            att_multi=self.att_multi,
            ksm_only_kernel_att=self.ksm_only_kernel_att,
            act_type=self.ksm_global_act,
            att_grid=self.att_grid,
            stride=self.stride,
            spatial_freq_decompose=self.spatial_freq_decompose
        )

        # 初始化FBM模块（如果需要）
        if self.kernel_size[0] in self.use_fbm_if_k_in:
            self.FBM = FrequencyBandModulation(self.in_channels, **self.fbm_cfg)
...
class Bottleneck_FDConv(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.cv1 = FDConv(c1, c_, k[0], 1)
        self.cv2 = FDConv(c_, c2, k[1], 1)
...
class C3k2_FDConv(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_FDConv(self.c, self.c, shortcut, g, k=(3, 3),
