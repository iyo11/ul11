...
class FGlo(nn.Module):
    ...
    def __init__(self, channel, reduction=16):
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(channel, mid, 1),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid, channel, 1),
            nn.Sigmoid()
        )
...
class ContextGuidedConv(nn.Module):
    ...
    改法1（只改这一条）：加入“局部保真残差”
      - 保留 stride=2 之后的 output 作为几何/边界基底
      - 融合特征 joi_feat 与 output 做残差相加，再进 FGlo
    """
    ...
    def __init__(self, nIn, nOut, dilation_rate=2, reduction=16):
        ...
        # 3) 上下文特征 (DW dilated 3x3)
        self.F_sur = nn.Conv2d(
            nOut, nOut, kernel_size=3, padding=dilation_rate,
            dilation=dilation_rate, groups=nOut, bias=False
        )
    ...
    def forward(self, x):
        # stride=2 base（局部/几何基底）
        base = self.conv1x1(x)  # (B, nOut, H/2, W/2)
        ...
        joi = self.reduce(self.act(self.bn(joi)))
