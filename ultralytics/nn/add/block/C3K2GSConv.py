...
class C3k_GSConv(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(GSConv(c_) for _ in range(n)))


class C3k2_GSConv(C2f):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k_GSConv(self.c, self.c, 2, shortcut, g) if c3k else GSConv(self.c) for _ in range(n)
        )
...
if __name__ == "__main__":
    ...
    print('GBConv_Input size:', input.size())  # 打印输入张量的形状
    print('GBConv_Output size:', output.size())  # 打印输出张量的形状

    input = torch.randn(1, 32, 64, 64)  # 创建一个形状为 (1,32,64, 64)
    GSConv = GSConv(32)
    output = GSConv(input)  # 通过GBSonv门控激活卷积模块
