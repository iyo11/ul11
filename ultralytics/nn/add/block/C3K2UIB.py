...
from ultralytics.nn.modules.block import Bottleneck, C2f, C3k, PSABlock
...
class C3k2_UIB(C2f):
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            nn.Sequential(
                UniversalInvertedBottleneckBlock(self.c, self.c),
                PSABlock(self.c, attn_ratio=0.5, num_heads=max(self.c // 64, 1)),
            )
            if attn
            else C3k(self.c, self.c, 2, shortcut, g)
            if c3k
            else UniversalInvertedBottleneckBlock(self.c, self.c)
            for _ in range(n)
        )